...
sums<-df%>%
  group_by(age, phase)%>%
  summarise(mean.dur=mean(mean.duration), 
            min.dur=min(mean.duration),
            max.dur=max(mean.duration),
            mean.rev=mean(mean.revisitation),
            min.rev=min(mean.revisitation),
            max.rev=max(mean.revisitation))
write.csv(sums, "output/time.use.stats.csv")
